...
export default function EditProduct({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      ...
      if (res.success) {
        ...
        setShowableImages(images);

        if (res.data.main_image) {
          const imageFile = await urlToFile(res.data.main_image);
          setMainImage({
            file: imageFile,
            url: res.data.main_image,
          });

          setShowedImage(res.data.main_image);
        }

        const attributes: Attribute[] = [];

        for (let attribute of res.data.attributes) {
          attributes.push({
            key: attribute.key,
  ...
  const handleDiscountPriceQuery = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (x.data) {
      ...
      console.log({ discountPriceValue: discountPriceValue });
                      ...
                      <div className='absolute inset-0 w-full h-full flex items-center pr-[130.5px] opacity-[0.01] hover:opacity-[1] transition-all ease-linear duration-200'>
                        <div
                          ...
                          onClick={() => {
                            ...
                            setAttributeKey(productAttributes[index].key);
                            setAttributeValue(productAttributes[index].value);
                            setIsAddAttributesOpen(true);
                          }}
                        >
                          <EditIcon color='#6695FF' />
